...
def calcula_frecuencias():  
    ...
    ######################################################################################################
    ## De todas las jugadas, almacenar los números y su frecuencia en el orden creciente de los números.##
    ######################################################################################################
    (los_numeros,las_frecuencias)=obtiene_frecuencias(numeros_distintos,matriz_de_datos)
    print("Los números: ", los_numeros)
    print("las frecuencias: ",las_frecuencias)
    
    nombre_archivo="C:/Users/58414/Django_curso/proyectosdjango/ProyectoLoteria/AppNumeros/programas/resultados/archivo_numeros_todas_jugadas.csv"    
    with open(nombre_archivo, 'w',newline='', encoding="utf-8") as csvfile:
            campos = ['Número', 'Frecuencia']
            writer = csv.DictWriter(csvfile, fieldnames=campos, delimiter=';')
            writer.writeheader()
            for j in range(0,len(los_numeros)):
                writer.writerow({'Número': str(los_numeros[j]), 'Frecuencia': str(las_frecuencias[j])})
    
    
    ######################################################################################
    ## Gráfica de las frecuencias de todos los números                                  ##
    ######################################################################################
    """
    titulo="Frecuencia de aparición de cada número"
    leyenda_x="Los distintos números"
    leyenda_y="Frecuencia"
    grafica_en_ventana_general(los_numeros,las_frecuencias,leyenda_x,leyenda_y,titulo)
    """
    ######################################################################################
    ...
    nombre_archivo="C:/Users/58414/Django_curso/proyectosdjango/ProyectoLoteria/AppNumeros/programas/resultados/archivo_todas_jugadas_ordenados_frecuencia.csv"    
    with open(nombre_archivo, 'w',newline='',encoding="utf-8") as csvfile:
            campos = ['Número', 'Frecuencia']
            #writer = csv.DictWriter(csvfile, fieldnames=campos, delimiter=';')
